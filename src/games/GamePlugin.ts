...
export interface GameConfig { mode: string; duration: number; seed: number; data: any; }
export interface PlayerInput { type: string; time: number; data?: any; }
...
// ===== TARGET TAP (30s) =====
interface Target { id: number; x: number; y: number; size: number; appearAt: number; duration: number; points: number; type: 'normal' | 'gold' | 'boss'; hitsRequired: number; }
...
export class TargetTapGame implements GamePlugin {
  ...
  generateConfig(): GameConfig {
    const seed = Date.now(); const rng = seededRandom(seed); this.targets = []; let id = 0;
    for (let t = 400; t < this.duration - 500; t += 250 + Math.floor(rng() * 200)) {
      const progress = t / this.duration; const phase = progress < 0.33 ? 1 : progress < 0.66 ? 2 : 3; const r = rng();
      let type: 'normal' | 'gold' | 'boss' = 'normal', points = 1, hitsRequired = 1, size = Math.max(5, 13 - progress * 7);
      if (r > 0.92 && phase >= 2) { type = 'boss'; points = 10; hitsRequired = 3; size = 16; }
      else if (r > 0.8) { type = 'gold'; points = 5; size = Math.max(4, 10 - progress * 5); }
      else { points = phase; size = Math.max(5, 14 - progress * 8); }
      this.targets.push({ id: id++, x: 8 + rng() * 84, y: 8 + rng() * 74, size, appearAt: t, duration: type === 'boss' ? 2500 : Math.max(800, 1800 - progress * 800), points, type, hitsRequired });
    }
...
export class ComboTapGame implements GamePlugin {
  ...
  generateConfig(): GameConfig {
    const seed = Date.now(); const rng = seededRandom(seed); const colors = ['red', 'blue', 'green', 'yellow'];
    this.steps = []; let id = 0, t = 600;
    while (t < this.duration - 400) {
      const progress = t / this.duration; const phase = progress < 0.33 ? 1 : progress < 0.66 ? 2 : 3;
      const timeLimit = Math.max(350, 1000 - progress * 600); const isRainbow = rng() > 0.9 && phase >= 2;
      this.steps.push({ id: id++, color: isRainbow ? 'rainbow' : colors[Math.floor(rng() * colors.length)], showAt: t, timeLimit, type: isRainbow ? 'rainbow' : 'normal' });
      t += timeLimit + 80;
    }
  ...
  processInput(playerId: string, input: PlayerInput): number {
    ...
    if (input.type === 'color_tap' && input.data) {
      ...
      if (idx < this.steps.length) {
        const step = this.steps[idx]; const correct = step.type === 'rainbow' ? true : input.data.color === step.color;
        if (correct) { this.combos[playerId]++; const combo = this.combos[playerId]; let bonus = Math.min(combo, 5); if (step.type === 'rainbow') bonus = 10; if (combo >= 10) bonus += 3; this.scores[playerId] += bonus; }
        else { this.combos[playerId] = 0; this.scores[playerId] = Math.max(0, this.scores[playerId] - 2); }
...
export class EnduranceGame implements GamePlugin {
  ...
  generateConfig(): GameConfig {
    const seed = Date.now(); const rng = seededRandom(seed); this.beats = []; let id = 0, t = 800, interval = 700;
    while (t < this.duration - 300) {
      const progress = t / this.duration; const window = Math.max(60, 180 - progress * 120); const r = rng();
      let type: 'normal' | 'double' | 'danger' = 'normal';
      if (r > 0.88 && progress > 0.3) type = 'danger'; else if (r > 0.75 && progress > 0.2) type = 'double';
      this.beats.push({ id: id++, time: t, window, type });
      if (type === 'double') { this.beats.push({ id: id++, time: t + 150, window, type: 'normal' }); t += 150; }
      interval = Math.max(250, 700 - progress * 450); t += interval;
    }
  ...
  processInput(playerId: string, input: PlayerInput): number {
    ...
    if (input.type === 'beat_tap') {
      const tapTime = input.time; let bestBeat: Beat | null = null, bestDiff = Infinity;
      for (const beat of this.beats) { if (this.beatHits[playerId].has(beat.id)) continue; const diff = Math.abs(tapTime - beat.time); if (diff < bestDiff && diff <= beat.window * 3) { bestDiff = diff; bestBeat = beat; } }
      if (bestBeat) {
        this.beatHits[playerId].add(bestBeat.id); const isDanger = bestBeat.type === 'danger';
        if (bestDiff <= bestBeat.window * 0.3) this.scores[playerId] += isDanger ? 8 : 3;
        else if (bestDiff <= bestBeat.window) this.scores[playerId] += isDanger ? 3 : 1;
        else this.scores[playerId] += 1;
      } else { this.scores[playerId] += 1; }
    }
    return this.scores[playerId];
  }
  getScore(playerId: string): number { return this.scores[playerId] || 0; }
  validate(playerId: string): boolean { return (this.inputs[playerId]?.length || 0) <= this.beats.length * 5; }
...
// ===== ARCHER BATTLE (30s) =====
export class ArcherBattleGame implements GamePlugin {
  mode = 'archer_battle';
  private scores: Record<string, number> = {};
  private inputs: Record<string, PlayerInput[]> = {};
  private duration: number;
  ...
  getScore(playerId: string): number { return this.scores[playerId] || 0; }
  validate(playerId: string): boolean { return (this.inputs[playerId]?.length || 0) <= 50; }
  getMaxScore(): number { return 30; }
}
...
export function createGame(mode: string, duration = 30000): GamePlugin {
  switch (mode) {
    ...
    case 'archer_battle': return new ArcherBattleGame(duration);
