...
export function setupSocket(io: Server) {
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('set_name', async (data: string | { token?: string; name?: string }) => {
      try {
        ...
        if (!user) {
          const pn = name || `Player_${Math.floor(Math.random() * 9999)}`;
          user = await UserService.findOrCreate(`dev_${socket.id}`, pn);
        }
    ...
    // === CANCEL QUEUE ===
    const handleCancel = () => { queue.remove(socket.id); socket.emit('queue_cancelled'); socket.emit('QUEUE_CANCELLED'); };
    ...
    const handleInput = (data: any) => {
      ...
      if (typeof data === 'string') {
        // Legacy tap event
        input = { type: 'tap', time: Date.now() - (match.startedAt || 0), data: {} };
      } else {
        input = { type: data.type || 'tap', time: data.time || (Date.now() - (match.startedAt || 0)), data: data.data || {} };
      }
      ...
      if (oppId) {
        io.to(oppId).emit('opponent_tap', newScore);
  ...
  setInterval(() => {
    ...
    expired.forEach(e => { const s = io.sockets.sockets.get(e.socketId); if (s) s.emit('queue_timeout'); });
  }, 5000);
...
async function doCreateMatch(io: Server, s1: Socket, p1: MatchPlayer, s2: Socket, p2: MatchPlayer, gameType: string) {
  ...
  try {
    ...
    s1.emit('error_msg', 'Match failed, refunded');
    s2.emit('error_msg', 'Match failed, refunded');
    return;
  }

  // Create game plugin + generate config
  ...
  const matchData = (you: string, opp: string) => ({
    matchId, gameType, opponent: opp, you, prize: prize.toFixed(2), gameConfig,
  });

  s1.emit('match_found', matchData(p1.name, p2.name));
  s2.emit('match_found', matchData(p2.name, p1.name));
  s1.emit('MATCH_FOUND', matchData(p1.name, p2.name));
  s2.emit('MATCH_FOUND', matchData(p2.name, p1.name));

  console.log(`⚔️ Match: ${p1.name} vs ${p2.name} | Mode: ${gameType} | ${matchId}`);
...
function startGame(io: Server, matchId: string) {
  ...
  if (!match) return;
  const data = { matchId, duration: GAME_DURATION, gameType: match.gameType };
  io.to(match.roomId).emit('game_start', data);
  io.to(match.roomId).emit('MATCH_START', data);
  console.log(`🎯 MATCH_START: ${matchId} | Mode: ${match.gameType}`);

  setTimeout(() => {
    const m = matchManager.get(matchId);
    if (m && m.status === 'ACTIVE') resolveMatch(io, matchId);
  }, GAME_DURATION + 500);
}
...
async function resolveMatch(io: Server, matchId: string) {
  ...
  const final1 = v1 ? s1 : 0;
  const final2 = v2 ? s2 : 0;

  if (final1 > final2) await endMatch(io, matchId, match.p1, match.p2, 'win');
  else if (final2 > final1) await endMatch(io, matchId, match.p2, match.p1, 'win');
...
async function endMatch(io: Server, matchId: string, winner: MatchPlayer | null, loser: MatchPlayer | null, reason: string) {
  ...
  try {
    ...
    if (reason === 'draw') {
      ...
      io.to(match.p1.socketId).emit('match_result', d(s1, s2));
      io.to(match.p2.socketId).emit('match_result', d(s2, s1));
      io.to(match.p1.socketId).emit('MATCH_RESULT', d(s1, s2));
      io.to(match.p2.socketId).emit('MATCH_RESULT', d(s2, s1));
      ...
      const wd = { result: 'win', yourScore: match.game.getScore(winner.socketId), oppScore: match.game.getScore(loser.socketId), prize, reason };
      const ld = { result: 'lose', yourScore: match.game.getScore(loser.socketId), oppScore: match.game.getScore(winner.socketId), prize: 0, reason };
      io.to(winner.socketId).emit('match_result', wd);
      io.to(loser.socketId).emit('match_result', ld);
      io.to(winner.socketId).emit('MATCH_RESULT', wd);
      io.to(loser.socketId).emit('MATCH_RESULT', ld);
      io.to(winner.socketId).emit('wallet_update', payout.newBalance);
      io.to(loser.socketId).emit('wallet_update', lw.balance.toNumber());
    }
  } catch (err) { console.error('endMatch DB error:', err); }

  const s1s = io.sockets.sockets.get(match.p1.socketId);
  const s2s = io.sockets.sockets.get(match.p2.socketId);
  if (s1s) s1s.leave(match.roomId);
  if (s2s) s2s.leave(match.roomId);
  console.log(`🏆 ${matchId} | ${reason === 'draw' ? 'DRAW' : `${winner?.name} wins`} | ${s1}-${s2}`);
}
